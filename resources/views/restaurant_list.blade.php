...
<body id="page-top">
    ...
    <div>
        <div class="row">
            ...
            <div class="col">
                <form method="post" action="{{ route('admin.add_restaurant_form') }}" >
                    @csrf
                    <button class="btn  btn-success mt-5"> + Add Restaurant </button>
                </form>
                <div>
                    <h2 class="mt-5 text-dark">Restaurants List</h2>
                    <table class="table table-striped" style="width:95%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><div class="text-center"><img src="{{ asset('storage/images/'. $restaurant->logo) }}" width="50px" height="50px"></div></td>
                                <td class="text-center col-4 align-item-center">
                                    <div class="d-flex align-items-center d-inline-block  ">
                                        ...
                                        @if($restaurant->deleted_at == NULL)
                                            <form method="post" action="{{ route('admin.disable_restaurant_record',$restaurant->id) }}">
                                                @csrf
                                                <input type="hidden" name="page" value="{{ $restaurant_list->currentPage() }}">
                                                <button class="btn  btn-danger btn-sm"> x Disable </button>
                                            </form>
                                         @else
                                            <form method="post" action="{{ route('admin.enable_restaurant_record',$restaurant->id) }}">
                                                @csrf
                                                <input type="hidden" name="page" value="{{ $restaurant_list->currentPage() }}">
                                        ...
                                        <form method="post" action="{{ route('admin.to_update_restaurant', $restaurant->id) }}"> 
                                            @csrf
                                            <input type="hidden" name="page" value="{{ $restaurant_list->currentPage() }}">
                                            <button class="btn btn-info btn-sm" {{ ($restaurant->deleted_at != null)?'disabled':'' }}> ✎ Update </button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
